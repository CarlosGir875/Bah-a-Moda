...
export default function AdminOrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pendiente': return <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-[10px] font-black uppercase tracking-widest border border-slate-200">📦 Pendiente</span>;
      case 'recibido': return <span className="px-3 py-1 rounded-full bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-widest border border-amber-200">✅ Confirmado</span>;
      case 'preparacion': return <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-[10px] font-black uppercase tracking-widest border border-blue-200">🛠️ Preparación</span>;
      case 'en_transito': return <span className="px-3 py-1 rounded-full bg-indigo-100 text-indigo-700 text-[10px] font-black uppercase tracking-widest border border-indigo-200">🚚 En Camino</span>;
      case 'listo_entrega': return <span className="px-3 py-1 rounded-full bg-green-100 text-green-700 text-[10px] font-black uppercase tracking-widest border border-green-200">🏠 Entregado</span>;
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
            {["todos", "pendiente", "recibido", "preparacion", "en_transito", "listo_entrega", "cancelado"].map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border relative ${
                    ...
                    <div className="flex gap-2">
                      {order.estado === 'pendiente' && (
                        <button 
                          ...
                          className="flex-1 bg-black text-white py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-md flex items-center justify-center gap-2 hover:scale-[1.02]"
                        >
                          <CheckCircle className="w-3.5 h-3.5" /> Confirmar Pedido (Generar ID)
                        </button>
                      )}
